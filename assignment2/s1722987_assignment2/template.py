...
class HMM:
    ...
    def tag(self, observations):
        ...
        for i in range(len(self.viterbi)-1,-1,-1):
...
P(x_i|c_i) equal P(x_i)P(c_i|x_i)/P(c_i)
           approximately equal  (1/P(c_i)) * sum(P(c_i|cc)P(cc|w_i-1) for cc in the tag set T )
This approach takes advantage of the existing lexicon knowledge, it will do better.""")[0:500]
